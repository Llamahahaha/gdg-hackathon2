...
import { 
  Search, Clock, Target, Activity, ChevronLeft, ChevronRight, Play, Pause, FileText, AlertTriangle, ZoomIn
} from 'lucide-react';
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={generateAudit} disabled={loading || timeline.length === 0} className="px-4 py-2 bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div className="flex-1 bg-black rounded-none border border-white/20 relative overflow-hidden group min-h-[400px]">
            {loading ? (
              ...
              <>
                ...
                {tacticalData?.output_video && (
                   <video 
                     className="w-full h-full object-cover opacity-60"
                     src={`/videos/${tacticalData.output_video.split('/').pop()}`}
                     id="replay-video"
                     muted
                     autoPlay
                     loop
                ...
                <div className="absolute bottom-8 left-8 p-4 bg-black/80 border border-white/20 backdrop-blur-xl space-y-1">
                  ...
                  <div className="text-sm font-bold font-orbitron">#00{frameIndex} / 00{timeline.length}</div>
          ...
          <div className="h-32 bg-black/40 border border-white/20 p-6 flex flex-col gap-4">
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-4">
                 <button onClick={() => setIsPlaying(!isPlaying)} disabled={loading || timeline.length === 0} className="w-8 h-8 bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 disabled:opacity-50">
               ...
               <div className="flex gap-2">
                 ...
                 <button onClick={() => setFrameIndex(Math.min(timeline.length - 1, frameIndex + 1))} disabled={loading} className="p-2 hover:bg-white/5 disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
               </div>
             </div>
             ...
             <div className="relative h-4 w-full bg-white/5 cursor-pointer group" onClick={(e) => {
               ...
               <motion.div 
                 animate={{ width: timeline.length > 0 ? `${(frameIndex / timeline.length) * 100}%` : '0%' }}
